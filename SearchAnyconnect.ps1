# Script pour chercher l executable du AnyConnect et l executer
# Mbaptiste le 20/12/2023
# v1

# Les deux chemins ou l executable peut se trouver
$FirstPath = "$env:ProgramFiles (x86)\Cisco\Cisco AnyConnect Secure Mobility Client\vpncli.exe"
$SecondPath = "$env:ProgramFiles\Cisco\Cisco AnyConnect Secure Mobility Client\vpncli.exe"

Write-host "Recherche du anyconnect en cours" -ForegroundColor DarkGreen

Try {
    # Teste si l executable est dans le premier chemin. Si il est ici, on l execute
    if (Test-Path $FirstPath) {
        Write-Host "AnyConnect trouve" -ForegroundColor DarkGreen
        Start-Process -FilePath $FirstPath -ErrorAction Stop
    }
    # Teste si l executable est dans le second chemin. Si il est ici, on l execute
    elseif (Test-Path $SecondPath) {
        Write-Host "AnyConnect trouve" -ForegroundColor DarkGreen
        Start-Process -FilePath $SecondPath -ErrorAction Stop
    }
    # Message d erreur si l executable n est pas trouve
    else {
        Write-Host "AnyConnect non trouve : Veuillez contacter le service informatique" -ForegroundColor DarkRed
		Read-host "Appuyez sur entree pour sortir"
    }
}
# Message d erreur si l execution du script s est mal passe
Catch {
    Write-Host "Une erreur s est produite lors du lancement de AnyConnect : $_" -ForegroundColor DarkRed
	Read-host "Appuyez sur entree pour sortir"
}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCB7gH9hjmz79/c2
# Hioi0A9fsxtmkElCATY8WWIohH6praCCE+cwggX6MIIE4qADAgECAhMaAAAAGFwU
...
# BAGCNwIBFTAvBgkqhkiG9w0BCQQxIgQglLPNEJFFLxntKplrjWqT4/edzTt5HOng
# KvTG9fJa5sYwDQYJKoZIhvcNAQEBBQAEggEAwfVSR/nKwz+wbkvlbEXUSaKnklbQ
# 5II9O8borpcj+9xUjhaO+9KQ6JX/8oK3YNz5zvITWxNQWMy6ENud7YbPeYQWU5eM
# 6vrfxonwiLH1UA2uYMhviBVQiJV3VHiGyQ6skXf5KrqeUw1PRCwL4TgXgpz0V1zr
# m7noA8w+iS9RnI/mi6kNY4IKgWJ4rzM6PQrZnsiqwGWBkeUwToa7OEGSUlIo1o1u
# xpRjtzBljIs5f4MYA0bXAXe+BPaV32iqlj7UoTEzHgDTy9Q5nlAD9N73z3eCPg4l
# RPfErNRVL9sad4sYSmWb5slYyhmoniLRjigqdL4dnR2LJywx4NLMHNv6WaGCA0sw
...
# BgkqhkiG9w0BCQMxCwYJKoZIhvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNDAxMDIx
# NDExMjBaMD8GCSqGSIb3DQEJBDEyBDDTdrC9fnJiis2EGRzUp5ZlJrcbY961KdDu
# ywQ3L5cXYQE9wRWNHE6sUulO45gc6qQwDQYJKoZIhvcNAQEBBQAEggIAXdiH6XeF
# iSH4pzzXPN69+/sOuexcY0CFyr/b7zOBhMFDofdbeyoTcQ/LVoMItZmRPyk8y7sX
# p2aHq02Ypd2DUH3VJdIN+pbI5B0yksLGdss+ISJblM8nxecqBhbr+pqL2hkecDQo
# 6zQVESc5zv8IUDPODXQ7dUOuXvbaEBZdYEUSIvEao7Q6onAUZxRFVJxEr6t3zzT2
# NYjGPHt0ZmRZoEuoFEWs14gGx+O101VWHFtoOBePevnyNPBuYZJm+9fXoP6ce28P
# qLolWqw5qwm1FLP12CEsFf4eZGnb4ob39xhq1YInpbefclWJf7a9Dork+ye9XumT
# YY7XsAOwvBwFimAbHLCaYgNMzPd4t9wYRY6Bq42IXcRAHTOiwr0Si9CMgCBtOr7d
# uNVoYNfOIsy6ZvIaYg+MurkZ4+VZYn2m/UPLZuuAHEq/8BLAZYfVGXBn2yt72VhA
# C9B2lj7zMy+ijZVi2NF7oMKbysM96yOu53/0n2CbAeR/LbiacBEI87vEwOtp8ON3
# KX9j+rivbTyA2Qbsnm4ckyBCQI4ry+q0wuvwbuQ3X7hZ5KRkOjddfUgRUKqNQzBa
# krPlfESckrCOgq8uHoHpcUbqJEfnbPXC6x0tmt/xXQy2r3NldirULDqoeQOIPBKL
# P+y6ZotW0rAQz9gaeMwiM4T0zHuavQXr+gI=
